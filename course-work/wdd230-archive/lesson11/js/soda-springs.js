...
const url = "https://api.openweathermap.org/data/2.5/weather?id=5607916&units=imperial&appid=86a8a127f039c280559b0b465179fac6";
const fUrl = "https://api.openweathermap.org/data/2.5/forecast?id=5607916&units=imperial&appid=86a8a127f039c280559b0b465179fac6";
...
.then((response) => response.json())
.then((jObject) => {
    const list = jObject['list']; 
    const fList = list.filter(list => list.dt_txt.includes('18:00:00'));
...
.then(function (jObject) {
    ...
    event.textContent = fTowns[0].events[0];
    event1.textContent = fTowns[0].events[1];
    event2.textContent = fTowns[0].events[2];
